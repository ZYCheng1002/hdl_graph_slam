...
class LoopDetector {
 public:
  ...
  /**
   * @brief 回环检测
   * @param keyframes       历史关键帧
   * @param new_keyframes   当前的一些关键帧
   * @param graph_slam      pgo
   */
 ...
 private:
  ...
  /**
   * @brief To validate a loop candidate this function applies a scan matching between keyframes consisting the loop. If
   * they are matched well, the loop is added to the pose graph
  ...
  Loop::Ptr matching(const std::vector<KeyFrame::Ptr>& candidate_keyframes,
    ...
    std::cout << "best_score: " << boost::format("%.3f") % best_score
              << "    time: " << boost::format("%.3f") % (t2 - t1).toSec() << "[sec]" << std::endl;
